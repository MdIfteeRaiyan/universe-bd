...
test("readiness supports universities with alternative GPA paths", async () => {
  ...
  assert.match(page, /GPA 2\.50 in both SSC and HSC, or GPA 2\.00 in one examination with combined GPA 6\.00/);
...
test("structured university rules take priority over legacy minimum GPA", async () => {
  ...
  assert.match(page, /LLB \(Hons\) requires combined SSC and HSC GPA 7\.00 or above/);
});
