...
export default function PublicDirectory() {
  ...
  return (
    <main className="site-shell min-h-screen bg-[#101827] text-slate-100">
      ...
      <section className="public-hero subpage-hero border-b border-slate-700 bg-[#121c2b]">
        <div className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-14">
          <p className="text-sm font-bold text-blue-400">PUBLIC UNIVERSITY GUIDE</p>
          <h1 className="editorial-title mt-3 max-w-3xl text-3xl font-bold tracking-tight sm:text-5xl">A separate, simpler public admission path.</h1>
      ...
      <section id="public-directory" tabIndex={-1} className="mx-auto max-w-7xl px-5 py-10 outline-none lg:px-8 lg:py-12">
        <div className="public-filter-bar surface-card rounded-2xl p-4 sm:p-5">
          <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_16rem_auto] md:items-end">
            <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-200">Search university or programme</span><span className="relative block"><Search className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2 text-slate-400" size={18} aria-hidden="true" /><input type="search" inputMode="search" autoComplete="off" value={query} onChange={(event) => setQuery(event.target.value)} className="field field-with-leading-icon min-h-12" placeholder="Search CSE, BUET, Chattogram..." /></span></label>
        ...
        {filtered.length ? <div className="mt-6 grid gap-5 md:grid-cols-2">{filtered.map((university) => (
          <article key={university.id} className="public-university-card content-defer surface-card rounded-2xl p-5 sm:p-6">
            ...
            <details className="programme-drawer mt-4 rounded-xl border border-slate-700 bg-[#111b2a] p-4"><summary className="flex min-h-11 cursor-pointer items-center justify-between gap-3 font-semibold text-slate-200">View all {university.programs.length} programmes <span aria-hidden="true">⌄</span></summary><ul className="mt-4 grid gap-2 text-sm text-slate-300 sm:grid-cols-2">{university.programs.map((programme) => <li key={programme} className="rounded-md bg-slate-700/50 px-3 py-2">{programme}</li>)}</ul></details>
        ...
        ))}</div> : <div className="mt-6 rounded-2xl border border-dashed border-slate-600 bg-[#172337] p-8 text-center"><Search className="mx-auto text-slate-500" aria-hidden="true" /><h3 className="mt-3 text-lg font-bold">No matching public university</h3><p className="mt-2 text-sm text-slate-400">Try another programme, university name or division.</p><button type="button" onClick={() => { setQuery(""); setDivision(""); }} className="mt-5 min-h-11 rounded-lg border border-slate-600 px-4 py-2 text-sm font-semibold text-slate-200 hover:border-blue-400">Show the full directory</button></div>}
      </section>
    </main>
  );
}
