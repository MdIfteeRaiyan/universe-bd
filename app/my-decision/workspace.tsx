...
export function DecisionWorkspace() {
  ...
  return (
    <main className="site-shell min-h-screen bg-[#101827] text-slate-100">
      ...
      <header className="premium-header border-b border-slate-700"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4"><Link href="/" className="quiet-action inline-flex min-h-11 items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold text-blue-200"><ArrowLeft size={16} aria-hidden="true" /> Home</Link><b>CampusChoice BD</b></div></header>
      ...
      <section id="decision-form" tabIndex={-1} className="mx-auto max-w-6xl px-5 py-10 outline-none">
        <div className="tool-panel rounded-2xl border border-slate-700 bg-[#172337] p-5 sm:p-7">
          ...
          <label className="mt-6 block text-sm font-semibold text-slate-200"><span className="flex justify-between gap-4"><span>Complete four-year family budget</span><b className="text-blue-200">{money(budget)}</b></span><input className="mt-3" type="range" min="500000" max="5000000" step="100000" value={budget} onChange={(event) => setBudget(Number(event.target.value))} aria-label="Complete four-year family budget" aria-valuetext={`${money(budget)} complete four-year family budget`} /></label>
          <button type="button" disabled={!programme} onClick={reveal} className="premium-action mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl px-5 py-3 font-bold text-white disabled:cursor-not-allowed disabled:opacity-40 sm:w-auto"><Sparkles size={18} aria-hidden="true" /> Show transparent matches</button>
        </div>

        {started && <div ref={resultsRef} tabIndex={-1} aria-live="polite" aria-label={`${matches.length} transparent university matches`} className="mt-10 scroll-mt-24 outline-none"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-bold text-blue-300">EXPLAINED RESULTS</p><h2 className="editorial-title mt-2 text-3xl font-bold">Your strongest visible fits.</h2></div><span className="text-sm text-slate-400">{matches.length} shown · not a universal ranking</span></div><div className="mt-6 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{matches.map(({ university, plan, reasons, fitCount }) => <article key={university.id} className="decision-result surface-card flex flex-col rounded-2xl p-5"><div className="flex items-start justify-between gap-3"><b className="rounded-lg bg-slate-950/40 px-3 py-2 text-xs text-blue-200">{university.short}</b><span className="text-xs font-semibold text-emerald-300">{fitCount}/4 visible checks</span></div><h3 className="mt-4 text-xl font-bold">{university.name}</h3><p className="mt-2 flex items-center gap-2 text-sm text-slate-400"><MapPin size={14} aria-hidden="true" /> {university.district}, {university.division}</p><ul className="mt-5 grid gap-2 text-sm">{reasons.map((reason, index) => <li key={reason} className="flex items-start gap-2 text-slate-300"><Check size={15} aria-hidden="true" className={`mt-0.5 shrink-0 ${index === 3 && !plan ? "text-amber-300" : "text-emerald-300"}`} /> {reason}</li>)}</ul><div className="mt-5 rounded-xl bg-slate-950/25 p-4"><p className="flex items-center gap-2 text-xs text-slate-400"><CircleDollarSign size={14} aria-hidden="true" /> Academic + living + 8% safety allowance</p><b className="mt-2 block text-lg text-blue-200">{plan ? `${money(plan.grandTotal.low)}–${money(plan.grandTotal.high)}` : "Verification pending"}</b></div><Link href={universityProfilePath(university)} className="quiet-action mt-5 inline-flex min-h-11 items-center justify-center rounded-lg px-4 py-2 text-sm font-semibold">Review the evidence</Link></article>)}</div></div>}
      </section>
    </main>
  );
}
